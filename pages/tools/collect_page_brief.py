...
``python3 pages/tools/collect_page_brief.py <mod> [options]``; part of the
``pages/`` authoring pipeline and not dispatched by any CLI.
...
Constraints: standard library only. The mods root defaults to the
``fuzs.multiloader.project.mods`` Gradle property and the pages root to
``<fuzs.multiloader.project.resources>/pages``, falling back to this
repository's own ``pages/`` directory. The branch directory must exist.

Usage:
    python3 pages/tools/collect_page_brief.py eternal-nether
    python3 pages/tools/collect_page_brief.py mutant-monsters --branch 26.2.x \
        --out pages/.authoring/mutant-monsters/brief.md
    python3 pages/tools/collect_page_brief.py air-hop --json
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--pages-root",
        help="pages data root (default: fuzs.multiloader.project.resources/pages, then this repo's pages/)",
    )
